...
├── check-usage-stats.ts         # 패턴 사용 통계 확인 ✨ NEW!
│
...
- **check-usage-stats.ts** - 패턴 사용 통계 확인 (실시간 활용도 추적)
...
프로젝트에는 골든 패턴의 실제 활용도를 추적하는 시스템이 구현되어 있습니다.

### 추적 방식

1. **자동 추적**: 대화 중 패턴이 사용되면 자동으로 `usage_frequency` 증가
2. **상세 로그**: `pattern_usage_logs` 테이블에 사용 기록 저장
   - 사용자 메시지
   - 유사도 점수
   - 관계 타입
   - 타임스탬프
...
다음 정보를 확인할 수 있습니다:
- 전체 사용 통계 (사용된 패턴 비율, 평균 사용 횟수)
- 가장 많이/적게 사용된 패턴 Top 10
- MBTI별 사용 통계
- 관계 타입별 사용 통계
- 카테고리별 사용 통계
...
**테이블**:
- `pattern_usage_logs` - 패턴 사용 로그
- `pattern_usage_stats` - 통계 뷰 (집계)

**함수**:
- `increment_pattern_usage()` - 패턴 사용 시 호출
...
```bash
npx tsx scripts/check-usage-stats.ts
```
